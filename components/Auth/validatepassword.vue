...
import { validateResetPassword, forgotPassword } from "~/services/authservices";
...
const { handleSubmit, defineField, errors } = useForm({
  ...
  initialValues: { ...form, username: route.params.email },
});
...
function resendOTP() {
  if (countdown.value === 0) {
    forgotPassword({ username: route.params.email }).then((res) => {
      if (res.status === 200) {
        // Start the countdown
